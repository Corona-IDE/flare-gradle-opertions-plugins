...
/**
 * Dependency matching specification which is designed to only match against other projects/modules in the same
 * multi-module system as the provided project
 *
 * <p>
 * Heavily based on default Gradle <a href=
 * "https://github.com/gradle/gradle/blob/master/subprojects/diagnostics/src/main/java/org/gradle/api/tasks/diagnostics/internal/dsl/DependencyResultSpec.java">"DependencyResultSpec"</a>
 *
 * @author romeara
 * @since 0.2.0
 */
public class ProjectDependencyResultSpec implements Spec<DependencyResult> {
    ...
    public ProjectDependencyResultSpec(Project project) {
        ...
        project.getRootProject().getAllprojects().forEach(p -> {
            projectIdentifiers.add(p.getPath());
    ...
    private boolean matchesRequested(DependencyResult candidate) {
        ...
        if (requested instanceof ModuleComponentSelector) {
            ...
            String requestedCandidate = requestedModule.getGroup() + ":" + requestedModule.getModule() + ":"
                    + requestedModule.getVersion();

            project.getLogger().debug("Comparing DependencyResult ${requestedCandidate} to known projects");

            return projectIdentifiers.stream()
                    .anyMatch(requestedCandidate::contains);
        }
    ...
    private boolean matchesSelected(ResolvedDependencyResult candidate) {
        ...
        return projectIdentifiers.stream()
                .anyMatch(selectedCandidate::contains);
    }
}
